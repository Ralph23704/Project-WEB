...
image VARCHAR(50) NOT NULL CHECK (LENGTH(name) <= 50),
price VARCHAR(20) NOT NULL CHECK (price REGEXP '^[0-9]+(\.[0-9]{1,2})? FCFA$'),
...
name VARCHAR(50) NOT NULL CHECK (LENGTH(name) <= 50),
description VARCHAR(50) NOT NULL CHECK (LENGTH(description) <= 50),
image VARCHAR(50) NOT NULL CHECK (LENGTH(image) <= 50),
